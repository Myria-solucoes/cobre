...
| ------------------------------ | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cut/wire.rs`                  | `CutRecord::to_bytes` / `CutRecord::try_from_bytes`                                 | Own version byte; direct version-reject test.                                                                                                                                     |
...
| `workspace.rs` `CapturedBasis` | `CapturedBasis::to_broadcast_payload` / `CapturedBasis::try_from_broadcast_payload` | Own version byte; direct version-reject test.                                                                                                                                     |
...
| `resolved_parameters`          | `resolved_parameters` encode/decode entry points                                    | Own version byte; direct version-reject test.                                                                                                                                     |

Enforced by `cargo test` + review. Not a bespoke CI script. The reject-test
note records, per format, whether the "reject/tolerate old version" obligation
is met by its own version byte, by delegation to another owner, or by
FlatBuffers forward-compat — so a future reader does not file a false "missing
test" finding for the delegated/forward-compat rows.
