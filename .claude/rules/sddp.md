...
A boundary stage — one whose `DisaggregationWeight::next_day_weight` is positive —
blends its water-balance RHS between its own (anchor) period's rate and the
next period's rate, day-weighted. The next period's rate is never read from
the current LP solve (it does not exist yet at patch time); it is always a
**peek**: `compute_disaggregation_next_rate` shifts the incoming AR lag by
inserting the anchor rate as the newest slot, then evaluates the next
period's PAR model with an explicit `next_eta`, never touching the current
stage's own state or LP columns. The peek is strictly scenario-local — it
reads only the caller's own `current_state`/`x_hat` and the sampler's
per-(iteration, scenario, stage) draw, so its result depends on nothing else
running concurrently, preserving determinism and declaration-order
invariance.

**Forward and simulation use the REALIZED next-period draw; backward uses
the CONDITIONAL MEAN.** Forward (`training::forward_pass_state`) and
simulation (`simulation::pipeline`) hold a sampler, so `next_eta` is an actual
`sample()` draw at the next period's representative stage
(`DisaggregationWeight::next_period_stage`) with that stage's own
`noise_group_id`. Backward (`training::backward::lp_setup`) evaluates a
fixed opening with no sampler, so `next_eta` is fixed at `0.0` — the model's
conditional mean, not a resample. **The two paths coincide bit-identically
only when there is a single opening** (equivalently, when the next period's
PAR model has `sigma == 0.0`, so the `eta` term vanishes regardless of its
value): a multi-opening stochastic setup makes the forward realization and
the backward mean legitimately different values, not a bug. Never assert
forward/backward equality outside that single-opening/zero-variance case.

**`z_inflow` carries the pure anchor rate; the blend is feed-forward, never a
fixpoint.** The peek's `anchor_rate` input is `z_inflow_rhs_buf`, populated
by `compute_water_balance_rhs` (the pre-blend core `transform_inflow_noise`
wraps) using the CURRENT stage's own realized/opening eta — never the
day-weighted blended value. `transform_inflow_noise`'s blend adds
`zeta_s[stage] * next_day_weight * (disagg_next_rate_buf[h] - z_inflow_rhs_buf[h])`
to the water-balance RHS (`noise_buf`) only; `z_inflow_rhs_buf` itself is
left untouched, so it keeps reporting the pure anchor rate downstream (lag
accumulation, `z_inflow` output) exactly as an interior stage would. This is
why every call site computing the peek's `anchor_rate` calls
`compute_water_balance_rhs` directly and never the `transform_inflow_noise`
wrapper: the wrapper's own blend step reads `disagg_next_rate_buf`, which does
not exist yet while the peek is populating it — calling the wrapper here
would apply a stale-or-empty blend before the real, single wrapper call that
follows.

**The lower bound applies the same conditional-mean blend as the backward
pass — the same "evaluate every per-opening transform, or understate the
bound" class as the NCS patch above.** `lb_evaluate_stage_0` runs the
identical peek (`compute_water_balance_rhs` then `compute_disaggregation_next_rate`
with `next_eta = 0.0`) inside its per-opening loop, since the lower bound holds no
sampler either — mirroring backward, never forward's realized draw. It uses
its own `disagg_shift_buf`, never `lag_matrix_buf`: unlike forward/backward
(which rebuild the truncation lag matrix fresh on every call),
`lb_evaluate_stage_0` precomputes that matrix ONCE outside the opening loop and
re-reads it every opening, so writing the peek's shift into the same buffer
would corrupt the next opening's truncation precompute. Skipping this blend at
the lower bound — like the NCS omission it mirrors — silently understates the
bound for a boundary stage 0.
Read: `stochastic/noise.rs` (`compute_water_balance_rhs`,
`transform_inflow_noise`, `compute_disaggregation_next_rate`, `has_par_model`),
`stochastic/lag_transition.rs` (`DisaggregationWeight`,
`precompute_disaggregation_weights`), `training/forward_pass_state.rs`
(`run_forward_worker`'s peek block), `simulation/pipeline.rs`
(`process_scenario_stages`'s peek block), `training/backward/lp_setup.rs`
(`patch_opening_bounds`'s peek block), `training/lower_bound.rs`
(`lb_evaluate_stage_0`'s peek block). Pinned by
`test_disaggregation_boundary_peek_equals_source_stage_rate` (the shifted-lag
evaluation matches a hand-derived expected value, never a re-read of the
function's own output — the anti-tautology pin for the shift/evaluate
arithmetic),
`test_disaggregation_backward_equals_forward_on_deterministic_case` (forward's
realized-eta peek and backward's eta-zero peek are bit-identical under
`sigma == 0.0`, the single-opening/deterministic case; explicitly NOT
asserted under a nonzero-variance multi-opening setup, where the two
legitimately diverge), and
`test_disaggregation_lower_bound_applies_boundary_blend` (the lower bound's
blended noise RHS matches a hand-derived conditional-mean value, independent
re-derivation, never a read of `lb_evaluate_stage_0`'s own arithmetic).
