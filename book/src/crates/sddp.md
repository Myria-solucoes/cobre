...
| `policy_load`         | `validate_policy_load`: single entry point for policy-load compatibility validation (`PolicyLoadKind::FullFcf`/`BoundaryInjection`); `build_basis_cache_from_checkpoint`, `load_boundary_cuts`, `inject_boundary_cuts`: policy loading for warm-start, resume, and terminal boundary cut injection from external checkpoints |
