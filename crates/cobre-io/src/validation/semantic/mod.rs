...
//! |49 | Override row `stage_id` a member of the declared nonnegative study stage id set (never a `[0, n)` position test), across thermal, hydro, line, pumping, contract, hydro unit group and generic constraint bounds, and bus, line, hydro and NCS penalties; NCS bounds keep their Layer-3 referential stage check | `constraints/*_bounds.parquet`, `constraints/penalty_overrides_*.parquet` | `BusinessRuleViolation` |
...
//! |53 | Explicit `spillage_discretization_points` is reserved and has no effect | `system/hydro_production_models.json` | `ModelQuality` (warning) |
//!
