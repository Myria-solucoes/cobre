//! Integration test: pre-horizon seed delivery at stage 0 with K=1 and
//! always-active fishing predicate.
//!
//! ## What this test asserts
//!
//! With `K = 1`, `n_stages = 5`, a single anticipated thermal (id=31), and
//! `past_anticipated_commitments.values_mw = [100.0]`, the LP must:
//!
...
//!    anticipated thermal costs $10/MWh vs the backup's $5000/MWh, and
//!    the per-block cost of the decision column is non-zero only at the
//!    decision stage (not zeroed), so the LP saturates commitment to avoid
//!    future backup dispatch.
//!
...
//!    - Stage 0: anticipated delivers seed (100 MW, zero cost) + backup 50 MW
//!      × $5000/MWh × 744 h = $186,000,000.
//!    - Stages 1–4 delivery: anticipated covers ≥ 150 MW load (zeroed cost).
...
//! never delivered to the LP; instead the backup had to cover all 150 MW,
//! producing a stage-0 cost of $558,000,000 — well above this test's bound of
//! $191,953,000.
//!
//! ## Entity IDs
//!
//! IDs 30 (hydro), 31 (anticipated thermal), 32 (backup thermal) are chosen to
//! be distinct from all existing anticipated tests:
...
//!
//! The 30-series ensures no cross-test entity confusion in nextest.
...
/// Stage-0 backup cost: backup carries 150 - 100 = 50 MW at $5000/MWh × 744 h.
/// The seed (100 MW) is delivered by the anticipated thermal at zero LP cost
/// (cost-zeroing predicate). Only the remaining 50 MW requires backup.
const STAGE_0_BACKUP_COST_USD: f64 = (150.0 - 100.0) * 744.0 * 5000.0; // $186,000,000

/// Upper bound on active-decision cost: 4 decision stages × 200 MW (max_gen)
/// × $10/MWh × 744 h = $5,952,000. The LP may commit less than max_gen if
/// cuts are loose after 1 training iteration, but will not exceed max_gen.
const MAX_DECISION_COST_USD: f64 = 4.0 * 200.0 * 744.0 * 10.0; // $5,952,000

/// Absolute cost tolerance matching `anticipated_numerical_reconciliation_k2`.
const COST_TOLERANCE_USD: f64 = 1_000.0;

/// Total upper bound: stage-0 backup + decision cost ceiling + tolerance.
...
/// Build a 5-stage K=1 system for the pre-horizon seed delivery test:
///
/// - 1 bus (deficit cost $5000/MWh, excess cost $0)
/// - 1 trivial hydro (id=30, 1 hm³ max, zero inflow) — keeps the model in the
///   thermal regime without adding hydro uncertainty.
/// - 1 anticipated thermal (id=31, K=1, cost $10/MWh, max_gen 200 MW)
/// - 1 backup thermal (id=32, cost $5000/MWh, max_gen 500 MW)
/// - Load 150 MW constant across all stages
/// - `past_anticipated_commitments = [(id=31, [100.0])]` — the 100 MW seed
///
...
/// intentional: the non-zero seed is a deliberate fixture for the always-active
/// fishing predicate test. The validator's rejection rule applies to JSON input
/// through `load_case`; integration tests that construct `System` directly are
/// exempt.
///
/// **Cost asymmetry**: anticipated ($10/MWh) vs backup ($5000/MWh) gives a
/// 500× ratio so the LP saturates anticipated dispatch at max_gen = 200 MW at
/// every active-decision stage.
///
...
fn build_system() -> cobre_core::System {
    ...
    // Anticipated thermal: K=1 lead stage, very cheap so the LP saturates
    ...
    // capacity is unavailable (stage 0: seed covers 100 MW, backup 50 MW).
    ...
    // Thermal index 0 = anticipated (id=31): cheap at $10/MWh, max 200 MW.
    // Thermal index 1 = backup (id=32): expensive at $5000/MWh, max 500 MW.
    ...
    // fishing constraint, committed_at(0) returns None, and the backup must
    // carry all 150 MW — producing a cost three times higher than the bound.
...
/// LP: the fishing equality at stage 0 pins the anticipated thermal to 100 MW
/// regardless of cut quality, and the cost-zeroing predicate removes any
/// double-counting. The cost bound is deliberately generous ($186M + $5.95M)
/// to accommodate a loose 1-iteration cut approximation.
...
/// in-study decisions correctly for stages 1–4.
///
/// ## Fixture
///
/// - K=1, n_stages=5
/// - Anticipated thermal id=31: cost $10/MWh, max_gen 200 MW
/// - Backup thermal id=32: cost $5000/MWh, max_gen 500 MW
/// - Load: 150 MW constant across all stages
...
///
/// **AC2** — `decision_at(t) > 0` and near 200 MW for t ∈ {0,1,2,3}: the LP
/// commits near max_gen because the anticipated thermal is 500× cheaper.
///
/// **AC3** — `committed_at(t) ≈ decision_at(t-1)` for t ∈ {1,2,3,4}: the
/// ring-buffer shift invariant — slot 0 at stage t carries the decision from
/// stage t-1.
///
/// **AC4** — `observed_total ≤ EXPECTED_TOTAL_UPPER_BOUND_USD`: stage-0 backup
/// cost ($186M) + decision cost ceiling ($5.95M) + $1000 tolerance.
///
...
fn pre_horizon_seed_delivers_at_stage_zero_k1() {
    ...
    // Step 3: define accessors for the anticipated thermal (id=31).
    ...
    // the LP commits near max_gen = 200 MW because the anticipated thermal is
    // 500× cheaper than backup. After 1 iteration the cut may be loose, but the
    // LP should still commit a non-trivial amount.
    for t in 0..4_usize {
    ...
    //   Decision cost ≤ 4 × 200 MW × 744 h × $10/MWh = $5,952,000.
    //   Tolerance = $1,000.
    //   Total upper bound = $191,953,000.
    //
    // If the seed is not delivered (regression), stage-0 backup covers 150 MW
    // instead of 50 MW: stage-0 cost becomes $558M ≫ $191.95M → AC4 fails.
