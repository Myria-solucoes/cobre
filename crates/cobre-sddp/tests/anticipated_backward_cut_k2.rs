...
//! `anticipated_state.start + 1*A + plant` (`state_to_lp_column`).
//!
...
// Closed-form expected coefficients at stage 0 FCF, in scaled cost units. Both
// slots carry the same magnitude, -c_reg / COST_SCALE * BLOCK_HOURS, via:
...
// ---------------------------------------------------------------------------
// StubComm — single-rank communicator
// ---------------------------------------------------------------------------
...
fn three_stage_k2_anticipated_cut_coefficient_propagates_correctly() {
    ...
    // via the Less-branch ring-buffer shift in state_to_lp_column. See module
    // docstring "Both state-fixing-row duals" section.
