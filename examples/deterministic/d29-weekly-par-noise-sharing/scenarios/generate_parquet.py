"""
Generate Parquet scenario files for d29-weekly-par-noise-sharing test case.
...
The inflow_history covers the single pre-study occurrence [2023-12-01, 2024-01-01)
at 200.0 m3/s — the stage-0 lag-1 seed, at full coverage so it casts to the same
value a recent_observations mirror would (no user-provided AR/stats file combo
here ever triggers history-based estimation, so this row seeds without estimating).
"""
...
#
# Single pre-study occurrence [2023-12-01, 2024-01-01) at 200.0 m3/s — the
# stage-0 lag-1 seed (full coverage, so it casts to 200.0 exactly).

history_hydro_ids = [0]
history_start_dates = [date(2023, 12, 1)]
history_end_dates = [date(2024, 1, 1)]
history_value_m3s = [200.0]
...
history_table = pa.table(
    {
        ...
        "value_m3s": pa.array(history_value_m3s, type=pa.float64()),
    },
