...
Topology
========
Cascade U (id 0) -> J (id 1): U carries travel_time_hours = 250.0 on its arc to
J. Stage 0 is 720 h resolved into 3 chronological blocks of 240 h each; stage 1
is a single 720 h block receiving whatever crosses the boundary. No
`load_factors.json` is supplied, so every block keeps the default 1.0 factor
(flat demand per block, no additional shaping). Both hydros have zero natural
inflow: the exercise is driven by U's 100 hm3 initial storage delayed through
the in-transit bucket, not by any inflow signal.
"""
...
#
# One bus (id 0); flat 1 MW at every stage (and, absent load_factors.json,
# every block within a chronological stage); std 0.0 (deterministic).
