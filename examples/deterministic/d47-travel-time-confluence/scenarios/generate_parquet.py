...
Topology
========
Two upstreams U1 (id 0) and U2 (id 1) both feed downstream J (id 2), each with
a DIFFERENT travel_time_hours (360.0 and 1080.0) over uniform 720 h stages, so
their delayed shares mature at different lags and must aggregate into J's
single bucket block rather than two separate ones. All three hydros have zero
natural inflow: the exercise is driven entirely by U1's and U2's 100 hm3
initial storages delayed through the shared in-transit bucket, not by any
inflow signal.
"""
...
#
# Zero inflow for all three hydros at every stage: the case is driven entirely
# by U1's/U2's initial storage and the shared travel-time bucket, not by any
# inflow signal.
inflow_hydro_ids = []
inflow_stage_ids = []
inflow_mean_m3s = []
...
#
# One bus (id 0); flat 1 MW at every stage, ample relative to whatever the
# cascade can ever deliver, so hydro never oversupplies a stage; std 0.0
# (deterministic).
