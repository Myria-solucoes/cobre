...
| [`external-scenarios-are-authoritative.md`](external-scenarios-are-authoritative.md)             | Proposal                                               | Under the `External` scheme the external scenario file is the sole source of a class's realized values (σ = 0 included); seasonal-stats/PAR become generation-only inputs. Fixes deterministic external load/NCS and AR(0) inflow; AR(p > 0) σ = 0 stays rejected |
